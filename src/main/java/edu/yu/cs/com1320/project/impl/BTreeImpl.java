...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    private Set<Key> PersManDocs = new HashSet<>();
        ...
        if(entry != null) {
            if(entry.getValue() != null) {
                if (entry.getValue() instanceof File) {
                    try {
                        entry.val = this.persistenceManager.deserialize(key);
    ...
    public Value put(Key key, Value val) {
        ...
        if (alreadyThere != null) {
            ...
            if (alreadyThere.getValue() != null) {
                if (alreadyThere.getValue() instanceof File) {
                    try {
                        temp = this.persistenceManager.deserialize(key);
                        this.persistenceManager.delete(key);
                    } catch (IOException e) {
                        throw new RuntimeException(e);
                    }
                } else {
                    temp = alreadyThere.getValue();
                }
    ...
    public void moveToDisk(Key k) throws Exception {
        if (k == null) {
            throw new IllegalArgumentException("Can't pass in null");
        }
        Entry old = this.get(this.root, k, this.height);
        if (old == null) {
            return;
        }
        if (old.getValue() instanceof File) {
            return;
        }
        persistenceManager.serialize(k, (Value)old.getValue());
        this.get(this.root, k, this.height).val = makeFilePath((URI)k);
    }

    private File makeFilePath(URI uri) {
